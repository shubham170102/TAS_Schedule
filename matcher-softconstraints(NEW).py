...
classWillRun = [LpVariable("C%dr"%c, 0, 1, LpInteger) 
                       for c in range(C)]
notSatisfyingMin = [LpVariable("C%dm"%c, 0, 1, LpInteger) 
                       for c in range(C)]
...
for c in range(C):    
    hardMin = sumStudentsInClass[c] - courseMins[c]*classWillRun[c] + notSatisfyingMin[c]
...
for c in range(C):
    ...
    objective += -penalty*notSatisfyingMin[c]
...
for c in range(C):
    ...
    classWillRun[c] = int(classWillRun[c].varValue)
    notSatisfyingMin[c] = int(notSatisfyingMin[c].varValue)
